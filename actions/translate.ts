"use server";
// this is for translating the language
...
async function translate(prevState: State, formData: FormData) {
  ...
  const { userId } = auth();// this is for authentication it only allows to translate the things is he is valid user
  //a valid use is a person who has logged in to the website
